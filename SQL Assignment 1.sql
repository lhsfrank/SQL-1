...
--4. Write a query that retrieves the columns ProductID, Name, Color and ListPrice from the Production.Product table, the rows that are not NULL for the column Color, 
...
SELECT Name + ' ' + Color AS NamenColor
FROM Production.Product
WHERE Color IS NOT NULL
...
SELECT Name, Color
FROM Production.Product
WHERE Color IS NOT NULL AND ProductID LIKE '3[17-22]%'
...
FROM Production.Product
WHERE Name LIKE 'A%' OR Name LIKE 'Seat L%' OR Name LIKE 'Seat P%'
ORDER BY Name
...
SELECT ISNULL(ProductSubcategoryID, 0), ISNULL(Color, 0)
FROM Production.Product